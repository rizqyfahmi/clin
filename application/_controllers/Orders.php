...
class Orders extends Admin_Controller
{
	...
	*/
		...
		foreach ($data as $key => $value) {
			...
			switch($value['paid_status']){
				...
				case '2':
					...
					break;
				case "3":
					$paid_status = '<span class="label label-error">Request Reject</span>';
					break;
				case '4':
	...
	public function fetchOrdersDataIn()
	{
		...
		foreach ($data as $key => $value) {
			...
			switch($value['status']){
				case '1':
					$status = '<span class="label label-warning">Laundry Received</span>';
					break;
				case '2':
					$status = '<span class="label label-success">Request Approved</span>';
					break;
				case "3":
					$status = '<span class="label label-error">Request Reject</span>';
					break;
				case '4':
					$status = '<span class="label label-warning">Send to Laundry</span>';
					break;
				case '5':
					$status = '<span class="label label-success">Laundry Received</span>';
					break;
				default:
	...
	*/
	public function approve()
	{
        ...
        $response = array();
        if($order_id) {
            $approve = $this->model_orders->approve($order_id,$status);
            if($approve == true) {
                $response['success'] = true;
